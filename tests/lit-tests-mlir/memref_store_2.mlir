...
module {
    func.func @test(
        ...
        memref.store %value, %mr0[%i, %j] {
            nontemporal = true
        } : memref<4x?xf32, strided<[?, ?], offset: ?>, 0>
        func.return
    }
}
...
// CHECK_CAN:           memref.store %arg2, %arg0[%arg1, %c1023] {nontemporal = true} : memref<4x?xf32, strided<[?, ?], offset: ?>>
// CHECK_CAN:           return
// CHECK_CAN:       }
// CHECK_CAN:   }
...
// CHECK_GEN:           "memref.store"(%arg2, %arg0, %arg1, %0) <{nontemporal = true}> : (f32, memref<4x?xf32, strided<[?, ?], offset: ?>>, index, index) -> ()
